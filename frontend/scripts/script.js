...
function initializeDeck() {
  ...
  for (let i = 0; i < 2; i++) {
    if (localStorage.getItem("bomba") === "true") {
      deck.push({ color: null, type: "wild", value: "bomba" });
    }
    if (localStorage.getItem("escudo") === "true") {
      deck.push({ color: null, type: "wild", value: "escudo" });
    }
    if (localStorage.getItem("robo") === "true") {
      deck.push({ color: null, type: "wild", value: "robo" });
    }
    if (localStorage.getItem("camaleon") === "true") {
      deck.push({ color: null, type: "wild", value: "camaleon" });
    }
    if (localStorage.getItem("rebote") === "true") {
      deck.push({ color: null, type: "wild", value: "rebote" });
    }
  }
  // Mezclar el mazo
    ...
    if (idx !== -1) {
      ...
      if (card.type === "wild") {
        if (players[playerIndex].isHuman) {
          ...
          const coloresEnMano = players[playerIndex].cards
            .map((c) => c.color)
            .filter((c) => ["red", "green", "blue", "yellow"].includes(c));
          if (coloresEnMano.length > 0) {
            currentColor =
              coloresEnMano[Math.floor(Math.random() * coloresEnMano.length)];
          } else {
  ...
  if (over) {
    alert(
      "FELICIDADES JUGADOR " +
        ...
        " has ganado esta ronda, ahora se mostrará el puntaje"
    );
...
function obtenerRutaImagen(card) {
  if (card.type === "number" || card.type === "special") {
    ...
    if (card.type === "special") {
      if (card.value === "draw2")
        return `images/Cartas/${color.carpeta}/+2_${color.sufijo}.png`;
      if (card.value === "reverse")
        return `images/Cartas/${color.carpeta}/reversa_${color.sufijo}.png`;
      if (card.value === "jump")
        return `images/Cartas/${color.carpeta}/comodin_salto_${color.sufijo}.png`;
    }
    // Cartas numéricas
  ...
  if (card.type === "wild") {
    if (card.value === "draw4")
      return "images/Cartas/comodines generales/+4_comodin.png";
    if (card.value === "wild")
      return "images/Cartas/comodines generales/cambio_color.png";
    if (card.value === "bomba")
      return "images/Cartas/comodines generales/carta_bomba.jpeg";
    if (card.value === "escudo")
      return "images/Cartas/comodines generales/escudo.jpeg";
    if (card.value === "robo")
      return "images/Cartas/comodines generales/robo.jpeg";
    if (card.value === "camaleon")
      return "images/Cartas/comodines generales/camaleon.jpeg";
    if (card.value === "rebote")
      return "images/Cartas/comodines generales/rebote.jpeg";
  }
  return "";
}
...
function turnoBot() {
  ...
  const top = discardPile[discardPile.length - 1];
  const cartaJugada = bot.cards.find(
    (card) =>
...
function mostrarTodasLasManos() {
  ...
  const primeraPosicion = document.getElementById(posiciones[0]);
  if (!primeraPosicion) {
    return;
  }
...
function updateGameStateUI(state) {
  ...
  // Solo actualizar la UI si estamos en la página del juego
  if (document.getElementById("zona-descarte")) {
    mostrarTodasLasManos();
    mostrarCartaDescarte();
    mostrarMazo();
    renderPoints();
...
function handleWebSocketEvent(msg) {
  ...
  if (msg.type === "round_score") {
    showNotification(
      `¡Fin de ronda! Ganador: ${msg.winner}. Puntos: ${
        msg.roundScore
  ...
  if (msg.type === "draw_penalty") {
    showNotification(
      `${msg.affectedPlayer} recibió una penalización de ${msg.amount} cartas`,
      "alert"
    );
  }
...
window.onload = () => {
  if (document.getElementById("btn-start-multiplayer")) {
    document
      ...
      .addEventListener("click", startGameClient);
  }
  if (document.getElementById("btn-join-game-multiplayer")) {
    document
      .getElementById("btn-join-game-multiplayer")
      .addEventListener("click", joinExistingGame);
  }
  if (document.getElementById("btn-restart-multiplayer")) {
    document
      .getElementById("btn-restart-multiplayer")
      .addEventListener("click", restartGameClient);
  }
};
